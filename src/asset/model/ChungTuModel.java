...
public class ChungTuModel {
	public static ArrayList<ChungTu> select(int month, int year) throws SQLException {
		PreparedStatement st = Database.connect()
				.prepareStatement("SELECT MaCT, NgayCT, Thang, Nam, DienGiai FROM ChungTu where Thang=? and Nam=?;");
		st.setInt(1, month);
		st.setInt(2, year);
	...
	public static boolean insert(String maCT, java.sql.Date ngayCT, int thang, int nam, String dienGiai)
			throws SQLException {
		PreparedStatement st = Database.connect()
				.prepareStatement("INSERT INTO chungtu(MaCT, NgayCT, Thang, Nam, DienGiai) VALUES (?, ?, ?, ?, ?)");
		st.setString(1, maCT);
		st.setDate(2, ngayCT);
		st.setInt(3, thang);
		st.setInt(4, nam);
